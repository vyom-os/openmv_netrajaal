...
class AppController:
    ...
    def radio_scan(self, target_addr=0):
        print(f"Checking radio connectivity with all neighbours...")
        self.create_and_send_message("radio_scan", {"message": f"Checking radio connectivity with all neighbours"}, timeout=0.5)
        async def _run_check():
            result = await self.apphandler.start_radio_scanning()
        try:
            loop = asyncio.get_event_loop()
            loop.create_task(_run_check())
        except Exception as e:
            logger.error(f"radio_check_new failed to schedule task: {e}")
